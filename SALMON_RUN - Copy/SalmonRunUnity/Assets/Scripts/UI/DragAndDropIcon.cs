...
public class DragAndDropIcon : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler {
    ...
    //cost of the tower to place one.
    static public float towerCost;
    ...
    public void CanBuy()
    {
        if(MoneyManager.bank < towerCost)
        {
            icon.GetComponent<Image>().color = new Color32(108, 108, 108, 255);
        }
        else
        {
            icon.GetComponent<Image>().color = new Color32(233, 233, 233, 255);
        }
