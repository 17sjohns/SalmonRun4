...
public class PostRunStatsPanelController : PanelController
{
    ...
    private void UpdatePanelData(List<FishGenome> parentGenomes, List<FishGenome> offspringGenomes)
    {
        ...
        titleText.text = "Turn " + (GameManager.Instance.Turn - 1) + " Summary";
        ...
        if (GameManager.Instance.Turn > 1)
        {
            ...
            {
                parentSmallText.SetText(smallDescriptor, FishGenomeUtilities.FindSmallGenomes(parentGenomes).Count);
                parentMediumText.SetText(mediumDescriptor, FishGenomeUtilities.FindMediumGenomes(parentGenomes).Count);
                parentLargeText.SetText(largeDescriptor, FishGenomeUtilities.FindLargeGenomes(parentGenomes).Count);
            }
            else
            {
            ...
            offspringSmallText.SetText(smallDescriptor, FishGenomeUtilities.FindSmallGenomes(offspringGenomes).Count);
            offspringMediumText.SetText(mediumDescriptor, FishGenomeUtilities.FindMediumGenomes(offspringGenomes).Count);
            offspringLargeText.SetText(largeDescriptor, FishGenomeUtilities.FindLargeGenomes(offspringGenomes).Count);
            offspringFemaleText.SetText(femaleDescriptor, FishGenomeUtilities.FindFemaleGenomes(offspringGenomes).Count);
            offspringMaleText.SetText(maleDescriptor, FishGenomeUtilities.FindMaleGenomes(offspringGenomes).Count);
        }
